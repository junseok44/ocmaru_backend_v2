...
// Spring AgendaController의 ExecutionTimelineResponseDto
// id, content, imageUrl만 응답하므로 그에 맞춘 타입
...
export default function AgendaToPolicy() {
  ...
  const {
    ...
    // Spring AgendaController: GET /agendas
    queryKey: ["/api/agendas"],
    queryFn: async () => {
      const res = await authFetch("/api/agendas", {
        cache: "no-store",
        headers: {
      ...
      // 기존 @shared/schema 기반 Agenda 타입과 맞추기 위해 normalize만 통과
      return normalizeApiResponse("/api/agendas", payload) as AgendaWithCategory[];
    },
  });
  ...
  const {
    ...
    // Spring CategoryController: GET /categories
  ...
  const filteredAgendas = useMemo(() => {
    ...
    if (periodFilter !== "all") {
      ...
      filtered = filtered.filter((agenda) => {
        const agendaDate = new Date(agenda.createdAt as string);
  ...
  const timelineQueries = useQueries({
    queries: displayedAgendas.map((agenda) => ({
      // Spring AgendaController: GET /agendas/{id}/execution-timeline
      queryKey: [`/agendas/${agenda.id}/execution-timeline`],
      queryFn: async () => {
        const res = await authFetch(
          `/agendas/${agenda.id}/execution-timeline`,
        );
      ...
      select: (data: ExecutionTimelineItem[]) => {
        // 백엔드에서 정렬 정보가 없다면 그대로 최신이 마지막이라고 가정
        return data;
      },
    })),
  });
...
function PolicyCard({ agenda }: { agenda: AgendaWithTimeline }) {
  ...
  const statusStyle = getStatusStyle(agenda.status ?? "created");
            ...
            <div className="mb-3">
              <div className="flex items-center gap-2 mb-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">최신 실행 내용</p>
                </div>
              </div>
